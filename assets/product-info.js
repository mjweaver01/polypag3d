if (!customElements.get('product-info')) {
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      fetchInventory(variant) {
        if (!variant?.id) return;
        const inventorySectionUrl = `${window.location.origin + window.location.pathname}?variant=${variant.id}`;

        fetch(inventorySectionUrl)
          .then((response) => response.text())
          .then((text) => {
            const nvDom = new DOMParser()
              .parseFromString(text, 'text/html')
            const inventoryQuantity = nvDom.querySelector('[data-selected-variant-inventory-quantity]')?.innerHTML;
            const inventoryPolicy = nvDom.querySelector('[data-selected-variant-inventory-policy]')?.innerHTML;

            if(inventoryQuantity <= 0 && inventoryPolicy === 'continue') {
              this.productForm?.toggleSubmitButton(false, false, true);
            }
